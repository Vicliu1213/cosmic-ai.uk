...
from marketbot.config.schema import DingTalkConfig
...
class MarketbotDingTalkHandler(CallbackHandler):
    """
    Standard DingTalk Stream SDK Callback Handler.
    Parses incoming messages and forwards them to the Marketbot channel.
    """
    ...
    async def process(self, message: CallbackMessage):
        ...
        try:
            ...
            # Forward to Marketbot via _on_message (non-blocking).
...
class DingTalkChannel(BaseChannel):
    ...
    async def start(self) -> None:
        ...
        try:
            ...
            handler = MarketbotDingTalkHandler(self)
    ...
    async def _send_markdown_text(self, token: str, chat_id: str, content: str) -> bool:
        return await self._send_batch_message(
            ...
            chat_id,
            "sampleMarkdown",
            {"text": content, "title": "Marketbot Reply"},
        )
    ...
    async def _on_message(self, content: str, sender_id: str, sender_name: str) -> None:
        """Handle incoming message (called by MarketbotDingTalkHandler).
