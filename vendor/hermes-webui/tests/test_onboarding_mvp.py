...
@_needs_yaml
def test_onboarding_setup_normalizes_localhost_base_url_shorthand():
    data, status = post(
        "/api/onboarding/setup",
        {
            "provider": "custom",
    ...
    assert status == 200
    assert data["system"]["current_base_url"] == "http://localhost:11434/v1"

    cfg_text = (_server_hermes_home() / "config.yaml").read_text(encoding="utf-8")
    assert "base_url: http://localhost:11434/v1" in cfg_text
