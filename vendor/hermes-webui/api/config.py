...
def normalize_base_url(raw: str | None) -> str:
    ...
    Accepts common local shorthand such as `11434`, `11434/v1`, or
    `localhost:11434/v1` and turns it into a full http URL.
    """
    ...
    if "/" in value:
        ...
        host_part, path_part = value, ""

    if host_part.isdigit():
        host_part = f"localhost:{host_part}"

    normalized = f"http://{host_part}"
    if path_part:
        normalized = f"{normalized}/{path_part}"
    return normalized
