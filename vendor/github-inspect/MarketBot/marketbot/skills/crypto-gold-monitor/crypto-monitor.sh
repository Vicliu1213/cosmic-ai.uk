...
cmd_all() {
    ...
    echo -e "${BOLD}═══════════════════════════════════════════════════════════════${NC}"
    echo ""
    echo -e "Exchange: 1 USD = ${YELLOW}¥${rate}${NC}"
    echo ""
    echo -e "${PURPLE}₿ Crypto${NC}"
    echo "─────────────────────────────────────────────────────"
    echo -e "  ${YELLOW}Bitcoin (BTC)${NC}"
    echo -e "      Price: ${GREEN}\$${btc_usd} / $(fmt_cny "$btc_usd" "$rate")${NC}"
    echo -e "      24h:   ${btc_clr}${btc_change}%${NC}"
    echo ""
    echo -e "  ${CYAN}Ethereum (ETH)${NC}"
    echo -e "      Price: ${GREEN}\$${eth_usd} / $(fmt_cny "$eth_usd" "$rate")${NC}"
    echo -e "      24h:   ${eth_clr}${eth_change}%${NC}"
    echo ""
    echo -e "${YELLOW}🥇 Metals${NC}"
    echo "─────────────────────────────────────────────────────"
    echo -e "  ${YELLOW}Gold (XAU/USD)${NC}"
    echo -e "      Price: ${GREEN}\$${gold}/oz / $(fmt_cny "$gold" "$rate")/oz${NC}"
    echo -e "      24h:   $(fmt_change "$gold_change")"
    echo ""
    echo -e "  ${CYAN}Silver (XAG/USD)${NC}"
    echo -e "      Price: ${GREEN}\$${silver}/oz / $(fmt_cny "$silver" "$rate")/oz${NC}"
    echo -e "      24h:   $(fmt_change "$silver_change")"
    echo ""
...
cmd_update() {
    ...
    echo -e "${GREEN}Updated metal anchors${NC}"
    echo "Gold: \$$gold/oz"
...
cmd_help() {
    ...
    echo ""
    echo "Usage: ./crypto-monitor.sh <command> [args]"
    echo ""
    echo "Commands:"
    echo "  all                    View all prices"
    echo "  update <gold> <silver> Update metals price anchors"
    echo "  refresh                Force refresh"
    echo "  help                   Show help"
}
