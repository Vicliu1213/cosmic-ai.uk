...
def show_price(symbol: str, info: dict) -> None:
...
def show_fundamentals(symbol: str, info: dict) -> None:
...
def show_history(symbol: str, ticker, period: str = "1mo") -> None:
...
def save_pro_chart(symbol: str, ticker, period: str = "3mo", chart_type: str = "candle", indicators=None):
    ...
    if indicators.get("vwap"):
        addplots.append(mpf.make_addplot(calc_vwap(hist), color="purple", width=1.0, panel=0))
    ...
    if indicators.get("rsi"):
        ...
        addplots.append(
            mpf.make_addplot(pd.Series(70, index=hist.index), panel=rsi_panel, color="gray", linestyle="--", width=0.7)
        )
        addplots.append(
            mpf.make_addplot(pd.Series(30, index=hist.index), panel=rsi_panel, color="gray", linestyle="--", width=0.7)
        )
...
def show_report(symbol: str, ticker, info: dict, period: str = "6mo") -> None:
    current = info.get("regularMarketPrice") or info.get("currentPrice")
    prev_close = info.get("regularMarketPreviousClose") or info.get("previousClose")
    ...
    upper, _, lower = calc_bbands(close)
    bb_pos = (close.iloc[-1] - lower.iloc[-1]) / (upper.iloc[-1] - lower.iloc[-1]) * 100
    macd, sig, _histo = calc_macd(close)
    macd_val = macd.iloc[-1]
    macd_sig = sig.iloc[-1]
    chart_path = save_pro_chart(symbol, ticker, period=period, indicators={"rsi": True, "macd": True, "bb": True})
...
[bold cyan]● Fundamentals[/bold cyan]
  Market Cap: {market_cap / 1e9:,.1f}B | Forward PE: {pe}
...
def main() -> None:
    ...
    if args_list and args_list[0] not in ["price", "fundamentals", "history", "pro", "chart", "report"]:
        args_list.insert(0, "price")

    args = parser.parse_args(args_list)
    ticker, info = get_ticker_info(args.symbol)
    ...
    if args.cmd == "price":
        show_price(args.symbol, info)
    elif args.cmd == "fundamentals":
        show_fundamentals(args.symbol, info)
    elif args.cmd == "history":
        show_history(args.symbol, ticker, period=args.period)
    elif args.cmd == "report":
        show_report(args.symbol, ticker, info, period=args.period)
    elif args.cmd == "pro":
        path = save_pro_chart(args.symbol, ticker, period=args.period, chart_type=args.chart_type, indicators=indicators)
        if path:
        ...
        if not hist.empty:
            ...
            if args.rsi:
                summary_parts.append(f"RSI: {calc_rsi(close).iloc[-1]:.1f}")
            if args.bb:
                upper, _, lower = calc_bbands(close)
                bb_pos = (close.iloc[-1] - lower.iloc[-1]) / (upper.iloc[-1] - lower.iloc[-1]) * 100
        ...
        path = f"/tmp/{args.symbol}_simple.png"
        plt.savefig(path)
        plt.close()
        print(f"CHART_PATH:{path}")
